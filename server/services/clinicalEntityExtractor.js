...
export const normalizeFacilityName = (rawFacility) => {
  ...
  // Strip outer brackets/parentheses
  clean = clean.replace(/^[\(\[\{]+|[\)\]\}]+$/g, '').trim();
  // Strip leading prepositions/directions
  clean = clean.replace(/^(?:inside|at|near|opp(?:\.|\s+)|opposite|beside)\s+/i, '').trim();
  // Strip leading/trailing bullet points or colons
  clean = clean.replace(/^[#*•\-\s:]+|[#*•\-\s:,;]+$/g, '').trim();
  if (clean.length < 4 || clean.length > 100) return null;

  // Title case words while preserving standard acronyms
  const words = clean.split(/\s+/);
  const normalizedWords = words.map(word => {
    if (/^[A-Z]{2,5}$/.test(word)) return word; // Preserve AIIMS, ESI, NIMS, OPD
...
export const extractClinicalEntities = (rawText = '', docTypeHint = null) => {
  ...
  // Pass 1: High priority clinical/visit/report/rx dates
  ...
  // Pass 2: Generic "Date:" label (strictly excluding Birthdate, Registration, Expiry, MFG)
  if (!rawDateCandidate) {
  ...
  // Pass 3: Fallback to any line containing a valid date pattern (excluding birthdate / registration lines)
  if (!rawDateCandidate) {
    for (const line of lines) {
  ...
  // Pass 1: Explicitly labeled facility patterns
  ...
  // Pass 2: Header lines (lines 0 to 12)
  ...
  for (const line of lines) {
    // Strictly reject chief complaint lines so complaints are never converted to diagnoses
    ...
    for (const pattern of diagnosisPatterns) {
      ...
      if (match && match[1]) {
        ...
        // Disallow generic placeholder phrases
